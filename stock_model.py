...
def score_analyze(model, x_test, y_test, y_open, y_close, model_name, ticker = 'SPY', task_type = 'classification', model_type = 'sk', commission = 0.01, drift = 0, lag = 100000, target = 'return'):
    ...
    if task_type == 'classification':
        analyze_df = pd.DataFrame(columns = ['ticker', 'model_name','accuracy', 'auc', 'realized_total_return', 'total_return', 'annual_return'])
        analyze_df.at[0, 'ticker'] = ticker
...
class stock_model:
    ...
    def __init__(self):
        ...
        self.para_exp = {'ticker':'ticker of the asset, default SPY. string',
